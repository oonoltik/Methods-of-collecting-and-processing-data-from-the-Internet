...
class InstagramSpider(scrapy.Spider):
    ...
    insta_login = 'newman_aleks' # userId	Instagram "42861538103" или 770905781
    insta_pwd = '#PWD_INSTAGRAM_BROWSER:10:1601543418:AZ1QAIJoiejYk/x67S9jfCTrJ0t5z8fBfV+4IRvpw8uxUvAMMeFRo1mx19zXVcpfYBXEv0z8c14WxWBWtKWeTH27lAJeTrWJeLQvaq1cnBVWxgBasV3LQaZpwPJGvNm9ZBqwX4tqANEsDbEAmA=='
    ...
    def user_posts_parse(self, response:HtmlResponse,username,user_id,variables):   #Принимаем ответ. Не забываем про параметры от cb_kwargs
        ...
        for post in posts:                                                                      #Перебираем посты, собираем данные
            item_post = InstaparserItem(
                user_id = user_id,
                photo = post['node']['display_url'],
                likes = post['node']['edge_media_preview_like']['count'],
                post = post['node']
            )
        yield item_post        #variables ч/з deepcopy во избежание гонок

                  #В пайплайн

# пишем отсюда собирать данные об их подписчиках и подписках.
    def user_followers(self, response:HtmlResponse):
        user_id = self.fetch_user_id(response.text, self.parse_user)  # Получаем id пользователя
        variables = {'id': user_id,  # Формируем словарь для передачи даных в запрос
                     'first': 12}
    ...
    def user_followers_parse(self, response:HtmlResponse,username,user_id, variables):
        ...
        print('item_follower', item_follower)
        yield item_follower
